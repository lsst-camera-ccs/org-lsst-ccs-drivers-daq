...
public class TestStats {
    ...
    /**
     * Print all DAQ Rms statistics data for specified Location Reads all
     * DAQRmsStatistics values and return them in table format.
     *
     * @param location Location in raft/board format, "R<nn>Reb<m>"
     * @return String reporting all data read
     * @throws DAQException
    ...
    public String readAllRms(@Argument(name = "location", description = "readout board in format R<nn>Reb<m>") String location) throws DAQException {
        DAQRmsStats rmsStats = stats.getDAQRmsStats(Location.of(location),
                Stats.Clear.NO);
        Map<String, Long> statMap = rmsStats.asMap();
        return makeStatsTable("Rms", statMap, location);
    }

    /**
     * Print all DAQ Rds statistics data for specified Location Reads all
     * DAQRdsStatistics values and return them in table format.
     *
     * @param location Location in raft/board format, "R<nn>Reb<m>"
     * @return String reporting all data read
     * @throws DAQException
    ...
    public String readAllRds(@Argument(name = "location", description = "readout board in format R<nn>Reb<m>") String location) throws DAQException {
        DAQRdsStats rdsStats = stats.getDAQRdsStats(Location.of(location),
                Stats.Clear.NO);
        Map<String, Long> statMap = rdsStats.asMap();
        return makeStatsTable("Rds", statMap, location);
    }

    /**
     * Print all DAQ Driver statistics data for specified Location Reads all
     * DAQDriverStatistics values and return them in table format.
     *
     * @param location Location in raft/board format, "R<nn>Reb<m>"
     * @return String reporting all data read
     * @throws DAQException
    ...
    public String readAllDriver(@Argument(name = "location", description = "readout board in format R<nn>Reb<m>") String location) throws DAQException {
        ...
        Map<String, Long> statMap = driverStats.asMap();
        return makeStatsTable("Driver", statMap, location);
    }

    /**
     * Print all DAQ Firmware statistics data for specified Location Reads all
     * DAQFirmwareStatistics values and return them in table format.
     *
    ...
    public String readAllFirmware(@Argument(name = "location", description = "readout board in format R<nn>Reb<m>") String location) throws DAQException {
        ...
        Map<String, Long> statMap = firmwareStats.asMap();
        return makeStatsTable("Firmware", statMap, location);
    }

    /**
     * Print and Clear all DAQ Rms statistics data for specified Location Like
     * readAllRms, but clears data after reading
     *
     * @param location Location in raft/board format, "R<nn>Reb<m>"
     * @return String reporting all data read
     * @throws DAQException
    ...
    /**
     * Print and Clear all DAQ Rds statistics data for specified Location Like
     * readAllRds, but clears data after reading
     *
     * @param location Location in raft/board format, "R<nn>Reb<m>"
     * @return String reporting all data read
     * @throws DAQException
     ...
     *
     * @param location Location in raft/board format, "R<nn>Reb<m>"
     * @param quantity of requested quantity
    ...
    public long readRmsStat(@Argument(name = "location", description = "readout board in format R<nn>Reb<m>") String location, @Argument(name = "statistic name") String quantity) throws DAQException {
        DAQRmsStats rmsStats = stats.getDAQRmsStats(Location.of(location), Stats.Clear.NO);
        return rmsStats.getStatistic(quantity);
    }
     ...
     *
     * @param location Location in raft/board format, "R<nn>Reb<m>"
     * @param quantity Name of requested quantity
     * @return Value of requested quantity
     * @throws DAQException
     */
    @Command(name = "readRdsStat", description = "read specified DAQ Rds statistic for specified locaation")
    public long readRdsStat(@Argument(name = "location", description = "readout board in format R<nn>Reb<m>") String location, @Argument(name = "statistic name") String quantity) throws DAQException {
        DAQRdsStats rdsStats = stats.getDAQRdsStats(Location.of(location), Stats.Clear.NO);
        return rdsStats.getStatistic(quantity);
    }
     ...
     * @throws DAQException
     */
    @Command(name = "readDriverStat", description = "read specified DAQ Driver statistic for specified location")
    public long readDriverStat(@Argument(name = "location", description = "readout board in format R<nn>Reb<m>") String location, @Argument(name = "statistic name") String quantity) throws DAQException {
        DAQDriverStats driverStats = stats.getDAQDriverStats(Location.of(location), Stats.Clear.NO);
        return driverStats.getStatistic(quantity);
    }
    ...
    public long readFirmwareStat(@Argument(name = "location", description = "readout board in format R<nn>Reb<m>") String location, @Argument(name = "statistic name") String quantity) throws DAQException {
        DAQFirmwareStats firmwareStats = stats.getDAQFirmwareStats(Location.of(location), Stats.Clear.NO);
        return firmwareStats.getStatistic(quantity);
    }
