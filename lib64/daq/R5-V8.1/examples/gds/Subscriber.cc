...
void Example::Subscriber::stop   (const GDS::StateMetadata& state)
{
  ...
  unsigned raw_stamps = _raw_stamps[state.sensor().index()];
  printf("  %s: %i raw stamps. Errors (xfer, size, beg, end) (%u, %u, %u, %u)\n", state.sensor().encode(), raw_stamps, err_xfer(state.sensor()), err_size(state.sensor()), err_miss_beg(state.sensor()), err_miss_end(state.sensor()));
}
