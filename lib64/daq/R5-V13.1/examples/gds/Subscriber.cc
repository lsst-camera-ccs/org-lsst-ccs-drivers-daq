...
void Example::Subscriber::raw_stamp(const GDS::StateMetadata& state, const GDS::RawStamp& stamp)
{
  ...
  char buf[16];
  printf("  Arrived: %016llx - Triggered: %016llx Sensor %s: Sequence %i Stamp %i %s %s %i bytes\n", (unsigned long long)(uint64_t)arr, (unsigned long long)(uint64_t)state.timestamp(), state.sensor().encode(buf), state.sequence(), _raw_stamps[state.sensor().index()], GDS::StateMetadata::type(state.type()), GDS::StateMetadata::status(state.status()), stamp.size());
}
