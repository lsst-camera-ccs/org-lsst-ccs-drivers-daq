...
{
public:
  enum Type   : uint8_t {STAMP=0, START=1, STOP=2, PAUSE=3, RESUME=4};
  enum Status : uint8_t {SUCCESS=0, BDI_ERROR=1};
public:
  static const char* type(Type);
  static const char* status(Status);
public:
  StateMetadata(const Type, uint16_t sequence, const OSA::TimeStamp&, const Location&, const Status status=SUCCESS);
